...
const BACKEND_URL = "https://api.inoxx.ai/";
...
export async function getAgentData(agentName: string): Promise<{ personality: string } | null> {
  try {
    const response = await fetch(`https://api.inoxx.ai/api/agent/${agentName}`);
    if (!response.ok) {
    ...
    console.error("Error fetching agent data from api.inoxx.ai:", (error as Error).message);
    return null;
  }
}
